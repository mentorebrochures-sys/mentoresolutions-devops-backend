...
exports.addContact = async (req, res) => {
  ...
  const { data, error } = await supabase.from("contacts").insert([{ email, mobile, instagram, linkedin }]).select();
  if (error) return res.status(500).json({ message: error.message });
...
exports.updateContact = async (req, res) => {
  ...
  const { data, error } = await supabase.from("contacts").update({ email, mobile, instagram, linkedin }).eq("id", id).select();
  if (error) return res.status(500).json({ message: error.message });
